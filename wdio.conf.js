...
const brows = require('./test/utils/browsers')
const ENV = process.env.ENV
let caps
if (!ENV || !['chrome', 'edge', 'firefox'].includes(ENV)) {
  console.log(
    'Starting Chrome. For other browser - please add ENV and run with\
     "npx cross-env ENV=(chrome | edge | firefox) npm run test)"'
  )
  caps = brows['chrome']
} else {
  caps = brows[process.env.ENV]
  console.log(
    `Starting ${process.env.ENV}. For other browser - please add ENV and run with\
     "npx cross-env ENV=(chrome | edge | firefox) npm run test)"`
  )
}
exports.config = {
  ...
  reporters: [
    ...
    [
      'junit',
      {
        ...
        // outputFileFormat: function (options) {
        //   return `report_${options.specs[0].match(/(\w+.e2e)/)[0]}.xml`
        // }
      }
    ]
  ],
  ...
  beforeSuite: function () {
    browser.addCommand(
      'parseResultsNumber',
      async function (searchResultsString) {
        let parsedResult = await Promise.resolve(
  ...
  afterTest: async function (step, scenario, { error }) {
    if (error) {
      ...
      await browser.saveScreenshot(
        `./screenshots/failed-tests/${step.parent.replace(
          /[^\w]/g,
          '_'
        )}_${timestamp}.png`
      )
